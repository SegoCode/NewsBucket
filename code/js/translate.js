...
const files = fs.readdirSync(DIR).filter(f => f.endsWith('_clusters_es.json'))

console.log(`→ Translating ${files.length} file(s) × ${LANGS.length} language(s)...`)
  ...
  for (const { code, prompt } of LANGS) {
    ...
    const res = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [{ role: 'system', content: prompt }, { role: 'user', content: JSON.stringify(clusters) }],
        temperature: 0.2,
        stream: true
      })
    })
    if (!res.ok) throw new Error(`${file}/${code}: HTTP ${res.status} ${await res.text()}`)
    ...
    if (!Array.isArray(translated)) throw new Error(`Invalid response for ${file}/${code}`)

    const outFile = path.basename(file, '_clusters_es.json') + `_clusters_${code}.json`
    fs.writeFileSync(path.join(DIR, outFile), JSON.stringify(translated, null, 2))
