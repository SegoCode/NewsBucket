...
for (const file of files) {
  const urls = fs.readFileSync(path.join(INPUT_DIR, file), 'utf-8').split('\n').map(l => l.trim()).filter(l => l && !l.startsWith('#'))
  ...
  const errors = results.flatMap((r, i) => r.status === 'rejected' ? [{ url: urls[i], error: r.reason.message }] : [])
  const output = { fetchedAt: new Date().toISOString(), totalItems: items.length, totalErrors: errors.length, items, errors }
  ...
  console.log(`✓ ${path.basename(file, '.txt')}: ${items.length} items, ${errors.length} errors`)
